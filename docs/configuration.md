...
`auto`의 post-launch cancel은 child bridge의 인증된 cooperative `ctx.abort()`/`ctx.shutdown()`뿐입니다. parent는 `pane.send_keys`, `pane.close`, rollback 또는 reaper mutation을 보내지 않습니다. child가 present/unknown/hung이면 recovery/manual cleanup 상태로 보존하고 late watcher를 유지하며, bounded `pane.list`가 terminal absence를 확인할 때만 retire합니다. `split`의 cancellation, focus, close, promotion/reaper는 exact recorded target만 대상으로 합니다. auto child terminal이 할당된 tab 밖으로 이동하면 manual focus도 current rebound target을 다시 검증한 뒤에만 가능합니다.
...
Herdr의 durable source 및 allocation record는 owner-only socket의 10진수 `dev`/`ino` generation에 결속됩니다. 교체된 socket, generation이 없는 legacy record, malformed list 응답, 실패한 요청 또는 잘못된 strict result discriminator는 **unknown**이며 launch·interrupt·close 또는 부재 보고를 허용하지 않습니다. 일치하는 `terminal_id`가 하나도 없는 유효하고 범위가 제한된 전역 `pane.list`만 부재를 증명합니다. `auto`는 범위가 제한된 printable `pi-subagent:<run-id>:<agent>` `tab_label`을 `layout.apply`에 전달합니다. 이 label과 관찰한 terminal title은 진단 메타데이터일 뿐 container 또는 lifecycle authority가 아닙니다. `tab.rename`이나 `tab.close`는 호출하지 않습니다.
