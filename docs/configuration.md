...
`herdr-pane`는 `HERDR_SOCKET_PATH`의 owner-only Unix socket에 직접 연결하며 Herdr CLI나 별도 supervisor process를 실행하지 않습니다. Herdr `v0.8.0`의 protocol **19**, preview의 protocol **20**, `v0.9.0`의 protocol **22**를 지원하며, `ping`으로 협상한 실제 revision을 durable intent·allocation·launch gate에 모두 기록합니다. 이후 allocation, launch delivery, focus, interrupt, close 직전에는 같은 revision을 다시 확인합니다. broker runtime의 executable provenance만 durable intent에 기록하고, owner-only Unix socket으로 세 revision에서 검증된 공통 subset인 `pane.get`/`pane.list`/`layout.apply`/`pane.split`/`pane.send_text`/`pane.send_keys`/`pane.close`, `agent.get`/`agent.wait`/`agent.focus`, `pane.report_metadata`를 처리합니다. `pane.focus`는 manual focus에 사용하지 않습니다. 지원 revision별 전용 field나 method는 다른 revision에 보내지 않으며, 그 밖의 revision은 거부합니다.
