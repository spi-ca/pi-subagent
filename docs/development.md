...
checkpoint schema는 v3입니다. routine의 `--max-cells=1..15` completed prefix만 resume할 수 있고, claim 뒤 각 provider cell 전에 terminalize되어 one-use입니다. concurrency partial resume과 automatic retry는 지원하지 않습니다. fixture 유효성은 routine 및 concurrency current-source verifier가 모두 성공할 때만 판정합니다. 문서 변경 뒤에는 네 source-bound fixture를 다시 생성한 뒤 이 두 verifier를 실행해야 하며, 그 전의 fixture를 최종 검증 결과로 간주하지 않습니다. 세부 계약은 [`interactive-runtime-performance-design.md`](./interactive-runtime-performance-design.md#m0-harness-상태)를 따릅니다.
