...
| core 격리 단위·통합 테스트 | `bun run test:core` | file-global Bun mock과 process global 때문에 각 inventory 파일을 순차적인 fresh OS process에서 실행하며, 아래 heavy 파일만 제외한다 |
...
| session-tail heavy | `bun run test:session-tail` | recent-ID cache의 4배인 4,096개 distinct assistant ID reverse replay; 과거 100,000-ID workload와 동일한 coverage를 주장하지 않는 CI 안정성 범위 |
...
### Lifecycle socket close의 Bun pathname 제한

Bun의 native Unix `server.close()`는 close 직전에 path를 다른 file로 바꿔도 현재 pathname을 unlink한다. lifecycle server는 mkdtemp가 만든 private `0700` directory와 generation marker/socket의 exact `(dev, ino)`를 close 전 검증하고, 정상 소유 socket은 native close 전에 best-effort로 unlink한다. 불일치 pathname은 private directory 안에서 quarantine/restore를 최선으로 시도한 뒤 listener와 모든 connection을 닫고 authority mismatch는 fail-closed로 directory를 retain한다. startup은 `mkdtemp` 직후 cleanup을 arm하고 marker를 exclusive handle로 만든 뒤 write 전에 `fstat` identity를 저장한다. 따라서 identity가 증명된 partial marker는 제거할 수 있지만, 첫 directory probe 또는 listener pathname identity를 증명할 수 없는 I/O failure는 artifact를 retain한다. Bun/Node API에는 inode-bound close/unlink가 없으므로 same-UID peer가 마지막 검증 뒤 unlink 또는 quarantine/restore 뒤 native close 전의 어느 순간에도 pathname을 교체하는 TOCTOU window를 완전히 막거나 foreign occupant cleanup을 보장할 수 없다. 이는 `0700` directory를 신뢰하는 non-hostile same-UID model의 residual I/O uncertainty이며, test seam은 startup failure와 native interleaving을 재현한다; hostile peer 경계에는 별도 UID/MAC sandbox가 필요하다.
...
| current compatibility | 1.4.2 | 선언된 모든 `@earendil-works/pi-*` devDependency를 정확히 0.85.1로 맞춘 임시 graph | `bun install --no-save` |

각 lane의 repository 설치 graph verifier는 Bun hoisted link와 `.bun` store의 nested symlink를 재귀 순회해 설치된 모든 `@earendil-works/pi-*`를 이름별 exact mapping과 대조한다. locked baseline mapping은 `0.84.4`, compatibility mapping은 `0.85.1`이며, 선택된 mapping의 모든 package는 정확한 버전으로 설치되어야 한다. 별도로 tarball smoke의 격리 consumer는 wildcard 또는 transitive drift를 막는 결정적 호환성 harness로서 선택된 전체 exact Pi graph와 선언된 non-Pi peer를 의도적으로 주입한다. 이는 최소 peer 설치를 증명하는 검사가 아니다. compatibility lane은 optional peer의 `*`를 해석에 맡기지 않고 임시 `package.json`에서 선언된 Pi 개발 패키지를 모두 exact `0.85.1`로 바꾼 뒤 manifest와 lockfile이 바뀌지 않았음을 검사한다. 두 lane 모두 선택된 Bun 버전과 `cc`의 위치·버전을 로그로 확인한다. 이 매트릭스는 hosted CI에서 구성되는 자동 검증이며, 여기서 로컬 재설치·다운로드나 provider/live acceptance를 실행한 증거는 아니다.
