...
| Test 목록 | 검증 범위 |
| --- | --- |
| `test/acceptance/performance-phase0-live.test.ts` | schema v4 two-tier plan: routine 15-cell/15-child exact order와 cmux concurrency 1-cell/16-child exact order, tier-specific acknowledgement/gate, schema v3 checkpoint/resume, source-bound record path, proof FIFO token/deadline, telemetry completeness와 identity-bound teardown, opt-in 15초 fixed/sanitized live-diagnostic watchdog(120초 unchanged-progress failure) 및 child-observation 전 bounded terminal diagnostic |
