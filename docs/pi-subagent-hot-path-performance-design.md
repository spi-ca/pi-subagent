...
- cmux control-v2 sibling launch는 canonical source `system.tree` 조회의 **동시에 진행 중인 fetch만** 공유한다. key는 shutdown generation, topology mutation generation, control socket canonical path/dev/ino, workspace UUID, surface UUID를 모두 포함한다. settle 즉시 폐기한다. 응답 중 topology mutation generation이 바뀐 observation은 즉시 채택하지 않고 뒤의 observation과 비교할 candidate로 보관한다. socket pathname generation 변경은 이 예외 없이 무조건 실패한다. generation-stable canonical observation은 이전 candidate와 달라도 채택한다. generation-invalidated observation은 identity를 기억하고 재시도하며, 뒤의 generation-invalidated canonical observation이 workspace UUID·surface UUID·pane UUID 모두에 대해 대소문자 무시로 정확히 같을 때만 채택한다. 다르면 새 invalidated identity를 기억해 attempt bound까지 계속하고, bound를 소진하면 `topology-mutated`로 fail-closed한다. parser는 invalid JSON, invalid shape, duplicate/cross-type identity, source absence를 구분하고 control adapter는 exit code와 bounded control code/state를 원문·credential·private socket path 없이 진단한다.
...
interactive child는 parent-owned 고정 lifecycle로 첫 정상 `agent_settled`에서 결과를 반환한 뒤 종료하므로 bounded queue와 prompt completion이 더 안전하다.
...
공통 M0 evidence contract는 [transport 설계의 M0 harness](./interactive-runtime-performance-design.md#m0-harness-상태)를 사용한다. 이는 `--record-local`로 재현했던 fixed local-child `1/4/8/16` barrier matrix의 **historical retained local capture**다; 현재 source binding 증거로 주장하지 않는다. provider/cmux/tmux mutation 없이 duration, settlement latency, child spawn/status-polling, parent CPU/RSS/event-loop, peak child 및 exact cleanup을 기록한다. fixture에는 revision, dirty flag와 tracked/untracked content·mode digest가 포함되어 verify가 현재 worktree와 exact match를 요구한다. 이 local evidence는 transport adapter 또는 provider live matrix의 증거가 아니며 cmux/tmux contract는 `not-applicable`로 명시한다. preflight/schema declaration과 persisted baseline verify는 별도 명령/검증이다. Gated provider live contract는 schema v4 two-tier다: `routine-v1`은 15 active-1 cells/15 children(historical planning estimate 약 5.7분), `cmux-concurrency-16-v1`은 cmux short-response active-runs=16 한 cell/16 children(historical planning estimate 약 8.2분)이다. 이 estimate는 fresh capture나 SLA가 아니다. 두 fixture 경로는 저장소에 포함되지만 explicit record로만 갱신하며, 현재 source/worktree identity에 대한 두 tier verifier가 모두 성공한 경우에만 live final-matrix evidence로 인정한다. 이 문서는 checked-in fixture의 current-source 검증 성공을 별도로 주장하지 않는다. evidence schema v4, checkpoint schema v4이며 routine checkpoint는 recorded Pi version과 current preflight Pi version의 exact equality 및 source/tier/plan binding 아래 one-use prefix resume만, concurrency는 partial resume 없이 지원한다. backend version은 evidence나 resume continuity contract에 기록하지 않는다. record의 exact gates/acks, terminalization과 no-automatic-retry rule, fixed fixture paths 및 aggregate verify는 canonical transport 문서를 따른다. automatic nightly/scheduled live workflow도 없다. local/smoke/chunk의 `4/8` 지원은 이 live contract를 확대하지 않는다.
...
- [ ] 100,000-entry fixture에서 tail/signature auxiliary in-memory state는 bounded recent cache를 지키며 exact on-disk index와 public message content RSS는 별도 측정한다. 이는 전체 acceptance TODO이며 현재 기본 CI evidence가 아니다.
...
- [x] Phase 6 exact tail/signature index (descriptor/path-bound 0700 bucket directory·0600 append-only exact ID records, bounded recent cache/Bloom, malformed UTF-8 fail-closed, arbitrary non-message ID 제외, 4 × `SESSION_TAIL_RECENT_ID_LIMIT` = 4,096 unique assistant ID의 cache eviction·reverse exact disk-index replay; public messages/callback 유지)
  - [ ] CI 안정성 보정 TODO: 기본 CI의 4,096-ID coverage는 initial/reverse drain의 count·session bytes·duration과 optional bucket count를 진단 로그로 남기며, 이전 100,000-ID workload와 동일한 coverage 또는 성능 증거를 주장하지 않는다. 100,000-ID evidence는 별도 측정이 필요하다.
...
- [x] Phase 7 local benchmark harness (`benchmark:phase7:*`): actual private marked 10,000-run enumeration/classification, 100,000-node graph, startup/total/RSS/event-loop/concurrency/mutation/cleanup을 record/verify한다. retained fixture는 historical capture이며 현재 source-bound evidence로 주장하지 않는다; revision/dirty/content-mode digest mismatch는 fail-closed하며 100,000 actual filesystem override는 opt-in이고 baseline에는 명시적으로 not-run
...
- [x] Stage 2 CI external-process boundary: `bun run ci`는 check 뒤 core와 세 heavy workload를 독립 stage로 실행한다. Linux CI stage는 Python 3 `/proc` supervisor가 session/process-group identity를 workload와 group cleanup이 끝날 때까지 보유하고, Bun 전달 original parent PID를 `PDEATHSIG` 직후 재확인해 orphan workload spawn을 거부하며, direct `Popen` child 상태를 소비하지 않고 adopted zombie만 selective `waitpid(pid, WNOHANG)`로 reap한다. timeout/signal에는 bounded TERM→KILL cleanup과 group-empty 확인을 수행한다. inventory는 repository-root Bun filename form을 정확히 한 stage에 배정하고 required heavy path의 missing/duplicate/empty partition을 fail-closed한다. stage coordinator는 inventory의 각 파일을 `--max-concurrency 1 --bail=1` fresh OS process에서 순차 실행하고, 각 bounded JUnit outcome을 합산한 하나의 final private raw report에서 pass와 heavy-stage skip을 확인한 뒤 삭제한다. 이 OS process 경계가 `--isolate` 대신 file-global Bun mock과 process global을 격리하며 allowlist telemetry는 schema/size 검증 후에만 upload한다. 이 boundary는 Bun의 in-process timeout이 filesystem I/O를 취소하거나 Node API만으로 PID TOCTOU를 완전히 제거한다고 주장하지 않으며 original test timeout/workload를 유지한다.
...
현재 terminal `subagent_result` producer details는 `kind: "subagent.background-result"`, `version: 1`, `jobId`, terminal `status`, 시각, `omittedBytes`의 closed schema다. versioned `omittedBytes`는 보존에서 빠진 producer 바이트의 provenance(cryptographic authentication은 아님)이며 유효한 trailing `[Background output truncated: N bytes omitted.]`와 대조한다. 명시적 legacy 네 필드(`jobId`, `status`, `startedAt`, `completedAt`) envelope는 읽기 좋게 렌더링하지만 first-party provenance를 부여하지 않고 suffix는 일반 비신뢰 본문으로 남긴다. `start`/`status`/`cancel`의 별도 versioned structured details와 이 renderer 모두 display-only다. 모델에 보이는 custom-message content나 기존 오류 의미를 바꾸지 않는다. producer는 현재·이전 경로에서 `crypto.randomUUID()`로 job ID를 만들므로 canonical lowercase v4 UUID 외 metadata는 인식하지 않고 fallback으로 처리한다. 펼친 본문은 12 KiB head, fallback은 4 KiB head를 유지하고 최대 64 logical line을 Pi의 ANSI-aware wrapper로 줄바꿈한 뒤 최대 96 rendered row만 layout에 넘긴다. wrapper의 일반 행은 재절단하지 않으며, 좁은 폭에서 단일 cell에 담을 수 없는 overwide 행만 `?`로 대체하고 clipping/substitution 표시를 남긴다. 따라서 모든 행은 폭을 넘지 않고 CJK·emoji가 조용히 사라지지 않는다. 행 예산이 소진되면 실제 job ID를 포함한 `... (output clipped • /subagent-result <job-id>)` footer를 남기므로 긴 한 줄의 suffix를 조용히 잃지 않는다. 이 방식은 wrapper의 보통 경로를 한 번 처리하는 것을 목표로 하지만, overwide 행 검증은 정확성 경계이며 별도 custom ANSI parser를 도입하지 않는다. 각 display component는 마지막 폭의 rendered row만 보관하고 `invalidate()`에서 버린다. process-global cache는 없다.

접힌 보기는 최대 4 KiB의 비신뢰 본문을 공통 `previewLines` visual-row budget으로 보여 준다. `app.tools.expand`의 host-configured hint를 사용하고 조회에 실패하면 `Ctrl+O`로 안전하게 fallback한다. `previewLines`는 전역 `pi-tool-display` config의 1–80 값이며 파일이 없거나 malformed·unsafe이면 기본 8이다. 왼쪽 클릭은 짧거나 긴 해당 카드만 토글하고, 전역 expand 상태 변경은 그 local choice를 덮는다. producer가 이미 붙인 canonical positive safe-integer trailing notice는 bounded suffix 검사 뒤 별도의 비신뢰 footer로 보호해 표시하며, current metadata의 `omittedBytes`는 `... (N B not retained)`로도 표시한다. UI의 `output clipped`과 producer의 not-retained/footer는 서로 구별한다. `/subagent-result`는 현재 branch의 strict envelope에서 보존된 text만 display-only editor로 열며 반환된 편집·취소는 버린다. 이 표시는 원문 전체의 보존·복구를 약속하지 않는다. fixture 기반 historical/current envelope, malformed UUID/control fallback, invalid notice digits, producer/UI clipping, config fallback, CJK·emoji 폭과 depth-disabled 등록을 focused test로 확인한다.
