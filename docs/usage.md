...
Herdr는 terminal backend와 `auto` layout의 user-visible 배치만 추가합니다. 공개 `subagent` **입력** schema/validation은 Herdr 이전 commit `7544439`와 동일합니다. 결과 envelope shape도 동일하며, 이미 병합된 Herdr 기능에서 `details.terminalMode`에 additive `herdr-pane`만 추가됐고 현재 작업은 추가 tool contract 변경을 만들지 않습니다.
